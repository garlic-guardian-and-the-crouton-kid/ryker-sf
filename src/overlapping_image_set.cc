...
CvPolygon CgalToCvPolygon(const ImageMetadata& image, const Polygon_2& face_polygon) {
  std::vector<cv::Point> points;
	std::transform(face_polygon.vertices_begin(), face_polygon.vertices_end(), std::back_inserter(points),
			[image](const Point_2& point) {
				return image.GeoToWarpedImage(cv::Point2f(CGAL::to_double(point.x()), CGAL::to_double(point.y())));
			});
