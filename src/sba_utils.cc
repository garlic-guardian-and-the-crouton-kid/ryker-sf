...
void quat2vec(double *inp, int nin, double *outp, int nout) {
  ...
  if (nin > 7) {
    // are they present?
    for (i = 0; i < nin - 7; ++i) outp[i] = inp[i];
  } else {
    i = 0;
  }
  ...
  // non-negative; if not, negate the quaternion since two quaternions q and -q
  // represent the same rotation
  mag = sqrt(inp[i] * inp[i] + inp[i + 1] * inp[i + 1] +
             inp[i + 2] * inp[i + 2] + inp[i + 3] * inp[i + 3]);
...
void vec2quat(double *inp, int nin, double *outp, int nout) {
  ...
  if (nin > 7 - 1) {
    // are they present?
    for (i = 0; i < nin - (7 - 1); ++i) outp[i] = inp[i];
  } else {
    i = 0;
  }
